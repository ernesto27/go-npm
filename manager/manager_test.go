...
func createMockDependencies(t *testing.T, baseDir string) *Dependencies {
	...
	return &Dependencies{
		...
		LifecycleManager:  scripts.NewLifecycleManager(cfg.LocalNodeModules, false),
	}
}
...
func TestYarnLockMigration(t *testing.T) {
	testCases := []struct {
		name        string
		setupFunc   func(t *testing.T) (*PackageManager, string, string)
		expectError bool
		...
		{
			name: "successfully migrates from yarn.lock v1 to go-npm-lock.json",
			setupFunc: func(t *testing.T) (*PackageManager, string, string) {
				t.Helper()
				pm, tmpDir, origDir := setupTestPackageManager(t)
...
is-even@^1.0.0:
  ...
  integrity sha512-FL3F5bNvAIV7lrW4fZGgP+gJdMFGmO5p5UzOhMNRd3Sjvws9P5K7ry3i3qNHQi7iQZD/a5wQJbPHzVPLqzGvbw==
  dependencies:
    is-odd "^0.1.2"
...
is-number@^3.0.0:
  ...
  integrity sha512-4cboCqIpliH+mAvFNegjZQ4kgKc3ZUhQVr3HvWbSh5q3WH2v82ct+T2Y1hdU5Gdtorx/cLifQjqCbL3fWc3L/g==

is-odd@^0.1.2:
  version "0.1.2"
  resolved "https://registry.yarnpkg.com/is-odd/-/is-odd-0.1.2.tgz#bc573b5ce371ef2aad6e6f49799b72bef13978a7"
  integrity sha512-Ri7C2K7o5IrUU9UEI8losXJCCD/UtsaIrkR5sL3JhRMD/07bPJfJhkRxbi48l7+j2I3M2YPk8NRZ0hmdlfiLpQ==
  dependencies:
    is-number "^3.0.0"

is-odd@^3.0.1:
  version "3.0.1"
  resolved "https://registry.yarnpkg.com/is-odd/-/is-odd-3.0.1.tgz#f4a6b7a4015f53d93dbcb1605e3ae1b4a3258546"
  integrity sha512-CQpnWPrDwmP1+SMHXZhtLtCv8v1eP88a4GBEIpqVoR78pqr9RsTi9KMzKx1icsXXN7LMTr0b4cMOH9Hq0g5wMg==
  dependencies:
    is-number "^3.0.0"
`
			...
			expectError: false,
			validate: func(t *testing.T, pm *PackageManager, tmpDir string) {
				// Verify go-npm-lock.json was created
				lockFilePath := filepath.Join(tmpDir, packagejson.LOCK_FILE_NAME_GO_NPM)
				assert.FileExists(t, lockFilePath, "go-npm-lock.json should be created")

				// Verify packageLock was populated
				assert.NotNil(t, pm.packageJsonParse.PackageLock)
				assert.NotNil(t, pm.packageJsonParse.PackageLock.Packages)
				assert.NotNil(t, pm.packageJsonParse.PackageLock.Dependencies)

				// Verify top-level dependencies are present
				assert.Contains(t, pm.packageJsonParse.PackageLock.Dependencies, "is-odd")
				assert.Contains(t, pm.packageJsonParse.PackageLock.Dependencies, "is-even")

				// Verify package entries exist in Packages map
				_, hasIsOdd := pm.packageJsonParse.PackageLock.Packages["node_modules/is-odd"]
				assert.True(t, hasIsOdd, "is-odd should exist in Packages")

				_, hasIsEven := pm.packageJsonParse.PackageLock.Packages["node_modules/is-even"]
				assert.True(t, hasIsEven, "is-even should exist in Packages")

				// Verify resolved URLs and integrity are preserved
				isOddEntry := pm.packageJsonParse.PackageLock.Packages["node_modules/is-odd"]
				assert.Contains(t, isOddEntry.Resolved, "registry.yarnpkg.com/is-odd")
				assert.NotEmpty(t, isOddEntry.Integrity)
			},
		},
		{
			name: "returns error for non-v1 yarn.lock format",
			setupFunc: func(t *testing.T) (*PackageManager, string, string) {
				t.Helper()
				pm, tmpDir, origDir := setupTestPackageManager(t)
  ...
  "dependencies": {
    "is-odd": "^3.0.1"
				...
				// Create invalid yarn.lock (not v1 format - missing header)
				yarnLockContent := `__metadata:
  version: 6
  cacheKey: 8
...
`
			...
			expectError: true,
			validate: func(t *testing.T, pm *PackageManager, tmpDir string) {
				// go-npm-lock.json should not be created
				lockFilePath := filepath.Join(tmpDir, packagejson.LOCK_FILE_NAME_GO_NPM)
				assert.NoFileExists(t, lockFilePath, "go-npm-lock.json should not be created for non-v1 yarn.lock")
			},
		},
		{
			name: "returns error when yarn.lock file does not exist",
			setupFunc: func(t *testing.T) (*PackageManager, string, string) {
				t.Helper()
				pm, tmpDir, origDir := setupTestPackageManager(t)

				// Create only package.json (no yarn.lock)
				packageJSONContent := `{
  "name": "test-project",
  "version": "1.0.0",
  "dependencies": {
    "is-odd": "^3.0.1"
			...
			validate: func(t *testing.T, pm *PackageManager, tmpDir string) {
				// go-npm-lock.json should not be created
				lockFilePath := filepath.Join(tmpDir, packagejson.LOCK_FILE_NAME_GO_NPM)
				assert.NoFileExists(t, lockFilePath, "go-npm-lock.json should not be created when yarn.lock doesn't exist")
			},
		},
		{
			name: "successfully migrates yarn.lock with scoped packages",
			setupFunc: func(t *testing.T) (*PackageManager, string, string) {
				t.Helper()
				pm, tmpDir, origDir := setupTestPackageManager(t)
  ...
  "dependencies": {
    "@types/node": "^18.0.0"
				...
				// Create yarn.lock v1 with scoped package
				yarnLockContent := `# THIS IS AN AUTOGENERATED FILE. DO NOT EDIT THIS FILE DIRECTLY.
# yarn lockfile v1
...
"@types/node@^18.0.0":
  version "18.19.0"
  resolved "https://registry.yarnpkg.com/@types/node/-/node-18.19.0.tgz#123456789"
...
`
			...
			validate: func(t *testing.T, pm *PackageManager, tmpDir string) {
				...
				// Verify scoped package is in dependencies
				assert.Contains(t, pm.packageJsonParse.PackageLock.Dependencies, "@types/node")

				// Verify scoped package entry exists
				entry, exists := pm.packageJsonParse.PackageLock.Packages["node_modules/@types/node"]
				assert.True(t, exists, "@types/node should exist in Packages")
				assert.Equal(t, "@types/node", entry.Name)
				assert.Equal(t, "18.19.0", entry.Version)
			},
		},
		{
			name: "successfully migrates yarn.lock with optional dependencies",
			setupFunc: func(t *testing.T) (*PackageManager, string, string) {
				t.Helper()
				pm, tmpDir, origDir := setupTestPackageManager(t)

				// Create package.json with optional dependency
				packageJSONContent := `{
  "name": "test-project",
  "version": "1.0.0",
  "dependencies": {
  ...
  "optionalDependencies": {
    "fsevents": "^2.3.2"
				...
				// Create yarn.lock v1 with optional dependency
				yarnLockContent := `# THIS IS AN AUTOGENERATED FILE. DO NOT EDIT THIS FILE DIRECTLY.
# yarn lockfile v1
...
fsevents@^2.3.2:
  version "2.3.3"
  resolved "https://registry.yarnpkg.com/fsevents/-/fsevents-2.3.3.tgz#123456789"
  integrity sha512-abcdefghijklmnop==
...
`
			...
			validate: func(t *testing.T, pm *PackageManager, tmpDir string) {
				...
				// Verify regular dependency is present
				assert.Contains(t, pm.packageJsonParse.PackageLock.Dependencies, "is-odd")

				// Verify optional dependency is tracked
				assert.NotNil(t, pm.packageJsonParse.PackageLock.OptionalDependencies)
				assert.Contains(t, pm.packageJsonParse.PackageLock.OptionalDependencies, "fsevents")
			},
		},
	}
		...
		},
		{
			...
			validate: func(t *testing.T, pm *PackageManager) {
				...
				if currentOS == "darwin" {
					...
					assert.True(t, exists, "fsevents should exist in packageLock.Packages on darwin")
					assert.NotEmpty(t, pkgItem.Resolved, "fsevents should have resolved URL on darwin")
				} else {
					// On non-darwin platforms, fsevents should be skipped
					// Check that it's either not in Packages or has empty Resolved
					pkgItem, exists := pm.packageLock.Packages["node_modules/fsevents"]
					if exists {
					...
					// If it doesn't exist in Packages, that's also acceptable behavior
				}
			},
		},
		{
			...
			validate: func(t *testing.T, pm *PackageManager) {
				...
				if currentOS == "linux" && currentCPU == "x64" {
					// On matching platform, package should be installed with resolved URL
					pkgItem, exists := pm.packageLock.Packages["node_modules/@esbuild/linux-x64"]
					assert.True(t, exists, "platform-specific package should exist in packageLock.Packages on matching platform")
					assert.NotEmpty(t, pkgItem.Resolved, "platform-specific package should have resolved URL on matching platform")
				} else {
					// On non-matching platform, package should be skipped
					pkgItem, exists := pm.packageLock.Packages["node_modules/@esbuild/linux-x64"]
					if exists {
						assert.True(t, pkgItem.Optional, "should be marked as optional")
					}
